...
import { Lightbulb, TrendingUp, Clock, Users, AlertCircle, Check } from 'lucide-react';

interface OtherTaskRecommendationsProps {
  taskData: {
    title: string;
    description: string;
    priority?: string;
...
export const OtherTaskRecommendations: React.FC<OtherTaskRecommendationsProps> = ({
  taskData,
    ...
    try {
      ...
      if (response.data) {
        ...
        if (Array.isArray(recommendationsData)) {
          // Filter out due date recommendations (they're shown above)
          const otherRecs = recommendationsData.filter(rec => rec.type !== 'due_date');
          setRecommendations(otherRecs);
        } else {
          setRecommendations([]);
        }
      } else {
